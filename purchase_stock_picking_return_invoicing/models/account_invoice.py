...
class AccountInvoice(models.Model):
    ...
    def _prepare_invoice_line_from_po_line(self, line):
        '''
        modificamos la cantidad y cuenta contables de las lineas de la factura antes de ser creadas.
        para que las lineas de nota de credito tengan la cantidad del campo a reembolsar.
        '''
        data = super(AccountInvoice,
                     self)._prepare_invoice_line_from_po_line(line)
        if line.product_id.purchase_method == 'receive':
            #TRESCLOUD: Esta formula aparentemente mejorada no funciona en nuestro caso
